...
func (self *RedisTrib) RebalanceClusterCmdNew(addr string, ws []string, useEmpty bool, threshold int, pipeline int, simulate bool, verbose bool, quietCheck bool) error {
	// var addr string
	...
	for _, e := range ws {
		logrus.Println("Weights on node: %s", e)
		if e != "" && strings.Contains(e, "=") {
			s := strings.Split(e, "=")
			node := self.GetNodeByAbbreviatedName(s[0])
			logrus.Printf("Node id is: %s", s[0])

			if node == nil {
				logrus.Printf("Node with ID %s is nil!", s[0])
			}
			if node == nil || !node.HasFlag("master") {
				logrus.Fatalf("*** No such master node %s", s[0])
			}
			logrus.Printf("Node master status: %s", node.HasFlag("master"))
	...
	//self.FixClusterCmdNew(addr, 60000)
	// Check cluster, only proceed if it looks sane.
	self.CheckCluster(quietCheck)
